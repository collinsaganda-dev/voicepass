...
-- Create a daily cleanup job (if cron is available)
DO $$
BEGIN
    IF EXISTS (
        ...
        PERFORM cron.schedule(
            ...
            '0 0 * * *',
            'CALL cleanup_inactive_sessions();'
        );
    ELSE
        RAISE NOTICE 'pg_cron extension not available. Skipping cleanup job setup.';
    END IF;
END
$$ LANGUAGE plpgsql;

-- Insert a demo session for testing (optional)
INSERT INTO sessions (room_code, name, organizer_id) 
VALUES (
    'DEMO2024', 
    'Demo VoicePass Session',
    uuid_generate_v4()
)
